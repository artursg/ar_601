...
%stepping
left=[-offset 0 0];
left = repmat(left,steps,1);
right=[offset 0 0];
right = repmat(right,steps,1);
...
for i=0:(t0/T-1)
    ...
    if k==1
        ...
        if left_flag==1
            zmp_y_next(i+1)=f_zmp_x(t,0,-right(k+1,1)+delta);
        else
            zmp_y_next(i+1)=f_zmp_x(t,0,-left(k+1,1)-delta);
        end
    else
        if left_flag==1
            zmp_x_next(i+1)=f_zmp_x(t,left(k,2)+zmp_x_offset*cos(left(k,3)),right(k+1,2)+zmp_x_offset*cos(right(k+1,3)));
            zmp_y_next(i+1)=f_zmp_x(t,-left(k,1)-delta-zmp_x_offset*sin(left(k,3)),-right(k+1,1)+delta-zmp_x_offset*sin(right(k+1,3)));
        else
            zmp_x_next(i+1)=f_zmp_x(t,right(k,2)+zmp_x_offset*cos(right(k,3)),left(k+1,2)+zmp_x_offset*cos(left(k+1,3)));
            zmp_y_next(i+1)=f_zmp_x(t,-right(k,1)+delta-zmp_x_offset*sin(right(k,3)),-left(k+1,1)-delta-zmp_x_offset*sin(left(k+1,3)));
        end          
    end
...
%dlmwrite('stepping.traj',result_matrix);
dlmwrite('~/catkin_ws/src/ar_601/ar601_trajectories/trajectories/stepping.traj',result_matrix);
